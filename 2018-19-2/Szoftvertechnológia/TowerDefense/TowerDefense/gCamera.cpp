...
/// </summary>
gCamera::gCamera(void) : m_eye(0.0f, 20.0f, 20.0f), m_at(0.0f), m_up(0.0f, 1.0f, 0.0f), m_speed(1.0f), m_goFw(0), m_goRight(0), m_slow(false)
{
	fi = M_PI / 2.0f; theta = M_PI / 4.0f ; dist = 200.0f;
	SetView( glm::vec3(0,0,dist), glm::vec3(0,0,0), glm::vec3(0,1,0));
...
gCamera::gCamera(glm::vec3 _eye, glm::vec3 _at, glm::vec3 _up) : m_speed(1.0f), m_goFw(0), m_goRight(0), m_dist(10), m_slow(false)
{
	fi = 0; theta = 0; dist = 200;
...
void gCamera::Update(float _deltaTime)
{
	...
	*/
	fi += m_goRight * _deltaTime * m_speed;
	theta += m_goFw * _deltaTime * m_speed;
	if (theta < 0.1f) theta = 0.1f;
	if (theta > M_PI * 0.45) theta = M_PI * 0.45;

	float x = dist * cosf(fi) * sinf(theta);
	float z = dist * sinf(fi) * sinf(theta);
	float y = dist * cosf(theta);
...
{
	dist -= 5 * m_speed * float(wheel.y);
	if (dist < 100) dist = 100.0f;
	if (dist > 280) dist = 280.0f;
}
