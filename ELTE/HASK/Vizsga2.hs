...
statementNoLRec
   ...
   <||> While <$> (token "while" *> lexeme expr)
             <*> statement
             <*  (token "endwhile")
   <||> flip Store <$> (lexeme ptr <* op "<-" <* lexeme (token "store" ) ) <*> (token " " *> lexeme expr)
   <||> flip Fetch <$> (var <* op "<-" <* lexeme (token "fetch" )) <*> (token " " *> lexeme ptr)
   <||> Update <$> ((lexeme (token "update ")) *> lexeme ptr) <*> ((token " ") *> lexeme expr) -- ?
   <||> Alias <$> (lexeme ptr <* op "~") <*> lexeme ptr --flip?
...
type Eval a = StateT RTState (ExceptT String Identity) a

newtype Ptr = Ptr Name
  deriving (Eq, Ord, Show)
...
ptr :: Parser Ptr
ptr = Ptr <$> lexeme identifier

tryM :: MonadError e m => Maybe a -> e -> m a
tryM (Just a) err = return a
tryM (Nothing) err = throwError err


runEval :: Eval a -> RTState -> Either String (a, RTState)
...
evalVar v = do
  (RTState vars ptrs mems allocated) <- get
...
evalWhile (Assign v expr) = do
  ...
  put (RTState vars' ptrs mems allocated)
...
evalWhile (Store expr ptr) = do
    val <- evalExpr expr
    (RTState vars ptrs mems allocated) <- get
    case Map.lookup ptr ptrs of
      Nothing ->
        let mems' = Map.insert allocated val mems in
        let ptrs' = Map.insert ptr allocated ptrs in
        let allocated' = allocated + 1 in
        put (RTState vars ptrs' mems' allocated')
      _ ->throwError "Pointer already defined"
evalWhile (Fetch ptr var) = do
  (RTState vars ptrs mems allocated) <- get
  case Map.lookup ptr ptrs of
    Nothing -> throwError "Pointer not found"
    Just cim -> 
      case Map.lookup cim mems of
        Nothing -> throwError "Location not found"
        Just ertek ->
          let vars' = Map.insert var ertek vars in
          put (RTState vars' ptrs mems allocated)
evalWhile (Update ptr expr) = do
  (RTState vars ptrs mems allocated) <- get
  var <- evalExpr expr
  case Map.lookup ptr ptrs of
    Nothing -> throwError "Pointer not found"
    Just cim -> 
      case Map.lookup cim mems of
        Nothing -> throwError "Location not found"
        _ ->
          let mems' = Map.insert cim var mems in
          put (RTState vars ptrs mems' allocated)
evalWhile (Alias ptr1 ptr2) = do
  (RTState vars ptrs mems allocated) <- get
  case Map.lookup ptr1 ptrs of
    Nothing -> 
      case Map.lookup ptr2 ptrs of
        Nothing -> throwError "Pointer not found"
        Just cim -> let ptrs' = Map.insert ptr1 cim ptrs in
                    put (RTState vars ptrs' mems allocated)
    _ -> throwError "Pointer already defined"
    ...
    -- | Alias Ptr Ptr
    --változók leképezése
    -- pointerek leképezésével memóriacímekre
    -- memória címek leképezésével futási idejú értékekre
